...
[![CI](https://github.com/alecnunn/shuriken/actions/workflows/ci.yml/badge.svg)](https://github.com/alecnunn/shuriken/actions/workflows/ci.yml)
...
* Windows and macOS are covered by CI, which runs the suite on
  `windows-latest` and `macos-latest` as well as Linux, so the Windows
  command-line quoting, the `CreateProcess`-style launcher, separator handling
  and the log formats are exercised on the real thing. Six tests are
  `#[cfg(unix)]`: they need a POSIX shell to write depfiles, sleep, or run a
  generator script. The Windows build can also be tested from Linux with
  [cargo-xwin](https://github.com/rust-cross/cargo-xwin) and
  [Wine](https://www.winehq.org/), which is the faster local loop (see
  [`dev/`](dev/)).
...
* Every push runs all of the above on Linux, Windows and macOS, against a
  pinned ninja 1.13.2, plus clippy, a 1.87 MSRV check and a packaging dry run.
  The Windows-only code paths can also be exercised from Linux under Wine (see
  [`dev/`](dev/)).
